// A pull feed: deploy BTC/USD with the pull path, sign an update off chain as the admin, and land it
...
const key = async (name: string) =>
  Keypair.fromSecretKey(Uint8Array.from(JSON.parse(readFileSync(new URL(`../keys/${name}-keypair.json`, import.meta.url), 'utf8'))));
...
if (!(await rpc.getAccountInfo(doppler.address))) {
...
const { signed } = await doppler.update(Date.now(), { price: 61_234_000_000n, conf: 10_000_000n, expo: -8 }).sign(admin);
