...
} from "../src/build-deploy-transaction.js";
...
test("buildDopplerDeployTransactions assembles loader v3 deploy transactions", async () => {
  ...
  const bundle = await buildDopplerDeployTransactions({
    connection,
    payer,
...
test("write transactions use a smaller chunk size when two signatures are required", async () => {
  ...
  const sharedAuthorityBundle = await buildDopplerDeployTransactions({
    connection: createMockConnection(),
    payer,
    programId: (await Keypair.generate()).publicKey,
    bytecode,
  ...
  const distinctAuthorityBundle = await buildDopplerDeployTransactions({
    connection: createMockConnection(),
    payer,
    upgradeAuthority,
    programId: (await Keypair.generate()).publicKey,
...
test("buildDopplerDeployTransactions compiles bytecode from generator config", async () => {
  const programKeypair = await Keypair.generate();
  ...
  const bytecode = compileDopplerBytecode(config);
