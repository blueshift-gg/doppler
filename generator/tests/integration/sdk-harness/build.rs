...
fn main() {
    ...
    let generator_dir = integration_dir
        .parent()
        .and_then(|tests| tests.parent())
        .expect("generator directory");
    ...
    rerun_if_changed(&generator_dir.join("src"));
    rerun_if_changed(&generator_dir.join("templates/rust"));
    ...
    let status = Command::new("bun")
        ...
        .status()
        .expect("failed to spawn bun; install Bun to run generator integration tests");

    if !status.success() {
        panic!("doppler generator artifact generation failed");
    }
