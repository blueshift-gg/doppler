...
function renderRustLib(config: GeneratorConfig, typeName: string): string {
  return [
    ...
    "}",
    "",
    `impl OraclePayload for ${typeName}Payload {`,
    "    const SIZE: usize = PAYLOAD_SIZE;",
    "",
    "    fn to_bytes(&self, bytes: &mut [u8]) {",
    "        assert!(bytes.len() == Self::SIZE);",
    `        bytes.copy_from_slice(&${typeName}Payload::to_bytes(self));`,
    "    }",
    "",
    "    fn from_bytes(bytes: &[u8]) -> Self {",
    '        let bytes: &[u8; PAYLOAD_SIZE] = bytes.try_into().expect("payload size checked");',
    `        ${typeName}Payload::from_bytes(bytes)`,
    "    }",
    "}",
    "",
